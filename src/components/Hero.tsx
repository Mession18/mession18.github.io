...
export function Hero() {
  ...
  return (
    <section className={`hero weather-${weather.kind} time-${weather.period}${weather.loading ? ' weather-pending' : ''}`} id="top">
      ...
      <div className="island-scene" aria-label="树木环绕的宁静海岛平原"><div className="hero-sun" aria-hidden="true" /><Moon /><div className="horizon-forest" aria-hidden="true">{Array.from({ length: 14 }, (_, index) => <i key={index} />)}</div><div className="knoll" aria-hidden="true"><i /><i /></div><div className="ground"><i /><i /><i /><i /><i /></div><div className="tree t1"><i className="trunk" /><span className="crown"><b /><b /><b /></span></div><div className="tree pine t2"><i className="trunk" /><span className="crown"><b /><b /><b /></span></div><div className="house"><i className="roof" /><i className="chimney" /><span className="gable" /><span className="attic-window" /><span className="window left-window" /><span className="window right-window" /><b className="door" /><span className="step" /></div></div>
