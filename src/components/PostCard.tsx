...
export function PostCard({ post, basePath = '/posts' }: { post: Post; basePath?: string }) {
  ...
  if (basePath === '/recipes') {
    ...
    return <article className={`post recipe-plate-card ${!recipeImage ? `recipe-eaten-card ${colorClass(post.color)}` : ''}`}><Link className="recipe-plate" to={`${basePath}/${post.slug}`} aria-label={`查看：${post.title}`}>
      ...
      <svg viewBox="0 0 360 220" aria-hidden="true"><defs><path id={`recipe-arc-${post.slug}`} d="M 72 118 Q 180 150 288 118" /></defs><text textAnchor="middle"><textPath href={`#recipe-arc-${post.slug}`} startOffset="50%">于{post.publishedAt}制作</textPath></text></svg>
    </Link></article>
  }
