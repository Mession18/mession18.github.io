...
export function ContentSectionPage({ section }: { section: ContentSectionKey }) {
  ...
  const visibleItems = section === 'recipes' && recipeTag !== 'all'
  ...
  return (
    <div className="page-surface">
      ...
      {section === 'recipes' && <div className="museum-filters recipe-filters" aria-label="菜谱标签">
        {['all', ...recipeTags].map((tag) => (
          <button className={recipeTag === tag ? 'active' : ''} key={tag} onClick={() => { setRecipeTag(tag); setPage(1) }}>
            {tag === 'all' ? '全部菜品' : tag}
          </button>
        ))}
      </div>}
