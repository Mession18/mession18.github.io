...
import { EmptyPlantingCard, EmptyRecipeCard, PostCard } from '../components/PostCard'
...
export function ContentSectionPage({ section }: { section: ContentSectionKey }) {
  ...
  const pageSize = section === 'recipes' || section === 'planting' ? 9 : 30
  const recipeTags = [...new Set(items.flatMap((item) => item.tags))]
  const visibleItems =
    (section === 'recipes' || section === 'planting') && recipeTag !== 'all'
  ...
  const emptySlots = section === 'recipes' || section === 'planting' ? Math.max(0, pageSize - pageItems.length) : 0
  return (
    <div className="page-surface">
      ...
      {(section === 'recipes' || section === 'planting') && (
      ...
      <section className="posts-library">
        <div className={`post-grid ${section === 'recipes' || section === 'planting' ? 'display-stand-grid' : ''}`}>
          {pageItems.map((item) => (
